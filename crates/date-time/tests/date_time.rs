...
#[test]
fn test_from_unix_timestamp_returns_err_for_out_of_range() {
...
fn test_roundtrip_via_string() -> anyhow::Result<()> {
    let original = DateTime::from_unix_timestamp(1700000000)?;
...
#[test]
fn test_reject_no_fractional_seconds_with_dot() {
    assert!("2021-02-03T16:17:18.000Z".parse::<DateTime>().is_err());
}

#[test]
fn test_ordering() -> anyhow::Result<()> {
    let a = DateTime::from_unix_timestamp(1000)?;
    let b = DateTime::from_unix_timestamp(2000)?;
    assert!(a < b);
    assert_eq!(a, a);
    Ok(())
}
...
fn test_add_duration() -> anyhow::Result<()> {
    ...
    assert_eq!(result.to_string(), "2021-02-03T16:18:18Z");
    Ok(())
}
...
fn test_sub_duration() -> anyhow::Result<()> {
    let dt: DateTime = "2021-02-03T16:17:18Z".parse()?;
    let result = (dt - Duration::from_secs(60))?;
    assert_eq!(result.to_string(), "2021-02-03T16:16:18Z");
    Ok(())
}
...
fn test_parse_before_epoch_rejected() {
    assert!("1969-12-31T23:59:59Z".parse::<DateTime>().is_err());
}
